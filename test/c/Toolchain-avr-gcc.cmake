...
SET(CMAKE_C_FLAGS "-fverbose-asm -save-temps")
...
macro(add_avr_test ELF_NAME FILES)
    # Set the target AVR device
    set(AVR_TARGET_DEVICE atmega32)

    # Set the GCC flags required
    set(CMAKE_C_FLAGS " -Wall -Os -mmcu=${AVR_TARGET_DEVICE}")
    set(CMAKE_VERBOSE_MAKEFILE true)
    set(CMAKE_C_OUTPUT_EXTENSION ".o")
    add_definitions(-DF_CPU=16000000UL)
    add_executable(${ELF_NAME}.elf ${FILES})
    target_link_libraries(${ELF_NAME}.elf)
    build_ihex(${ELF_NAME} ${CMAKE_CURRENT_BINARY_DIR})
endmacro(add_avr_test)

macro(build_ihex ELF_NAME BINDIR)
    add_custom_target(build_hex_${ELF_NAME} ALL 
    COMMAND avr-objcopy -j .text -j .data -O ihex ${ELF_NAME}.elf ${ELF_NAME}.hex 
   ...
   add_custom_target(listing_${ELF_NAME} ALL
	COMMAND avr-objdump -s -D ${ELF_NAME}.elf > ${ELF_NAME}.lss
    DEPENDS ${ELF_NAME}.elf
    WORKING_DIRECTORY ${BINDIR}
    VERBATIM
    )
